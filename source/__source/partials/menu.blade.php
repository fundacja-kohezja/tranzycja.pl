        ...
        <div class="hidden lg:block lg:ml-6">
          <div class="items-center h-full flex space-x-4 font-heading font-semibold tracking-wider">
            @foreach ($items as $item)
                <a href="{{ $item->path }}"
                    @if($item->accented ?? false)
                      class="text-gray-700 dark:text-gray-300 bg-white hover:bg-gray-200 dark:bg-gray-850 dark:hover:bg-gray-900 hover:text-blue-400 dark:hover:text-indigo-300 px-3 py-2 rounded-md text-sm border-0{{ $page->isActive($item->path) ? ' font-extrabold' : ''}}"
                    @else
