...
<div class="container max-w-6xl mx-auto px-6 py-4">
    <div class="text-sm flex items-center mt-1 float-right text-indigo-700 dark:text-indigo-300">
        ...
        <span class="align-middle">{{ (new \DateTime($page->data))->format('j M Y') }}</span>
    </div>
    <div class="text-pink-600 text-xl dark:text-indigo-300">
        <nav>
            <span href="/publikacje" class="border-b-0 font-heading font-bold tracking-wider">
    ...
    <main class="flex flex-col lg:flex-row clear-both">
        <article class="long-article title-wide DocSearch-content w-full lg:w-3/5 break-words pb-16 justify-paragraphs">
            @yield('content')
        </article>
    </main>
</div>
