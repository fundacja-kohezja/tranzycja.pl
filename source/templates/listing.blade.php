...
<main class="container max-w-6xl mx-auto px-6 py-10 md:py-12">
    <div class="text-center">
        <h1 class="-purple-700 dark:text-purple-300 mb-0">
                ...
                <a class="excerpt-card flex flex-grow border-b-0 bg-gray-200 hover:bg-indigo-100 dark:bg-gray-800 dark:hover:bg-blue-900 shadow rounded-lg break-words px-4 py-6" href="{{ $poradnik->getUrl() }}">
                    <article class="flex flex-grow flex-col">
                        ...
                        <div class="mt-2 text-indigo-700 dark:text-indigo-300">
                            ...
                            <span class="align-middle">{{ Jenssegers\Date\Date::createFromTimestamp($poradnik->getModifiedTime())->format('j F Y') }}</span>
                        </div>
    ...
    <div class="text-center">
        <nav class="max-w-full relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Paginacja">
                ...
                <div class="flex">
                    @foreach ($pagination->pages as $pageNumber => $path)
                        @if($pagination->currentPage == $pageNumber)
                            <span class="md:inline-flex relative items-center px-4 py-2 border border-gray-300 dark:border-gray-700 bg-white dark:bg-black text-sm font-extrabold text-purple-700 dark:text-purple-400 hover:bg-gray-50">
                                {{ $pageNumber }}
                            </span>
                        @else
            ...
            @if ($next = $pagination->next)
                <a href="{{ $page->baseUrl }}{{ $next }}" class="bg-gray-200 dark:bg-gray-800 inline-flex items-center px-2 py-2 rounded-r-md border border-gray-400 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-indigo-100 dark:hover:bg-indigo-800 hover:text-indigo-800 dark:hover:text-white px-3 py-2 text-sm font-medium border-0">
